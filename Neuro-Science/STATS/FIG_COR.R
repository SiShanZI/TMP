...
TMPj <- pcor.test(TMP_DF[, i+7], TMP_DF$DD, c(TMP_DF$AGE, TMP_DF$GENDER, TMP_DF$CENTER))
if(TMPj$p.value <= 0.05){IDX2[[j]] = c(IDX2[[j]], i)}
}
}
}
...
FLAG <- FLAG + 1
GP <- gpar(fontsize = 13, fontface = 4)
grob1 <- grobTree(textGrob(TEXT_0, x = 0.1,  y = 0.95, just = 0, gp = GP))
grob2 <- grobTree(textGrob(TEXT_1, x = 0.1,  y = 0.85, just = 0, gp = GP))
grob3 <- grobTree(textGrob(TEXT_2, x = 0.1,  y = 0.75, just = 0, gp = GP))
#
## x = EDSS, DD ##
FIG <- ggplot(data = RES, mapping = aes(x = DD, y = RES[, i+7], color = GROUP)) +
theme_bw() +
geom_point(size = 3) +
geom_smooth(method = 'lm', se = FALSE, size = 2) +
## x = 'EDSS', 'DD'; y = expression('Volume / mm' ^ 3), 'FA', 'MD', 'RD' ##
labs(x = 'DD', y = 'RD', title = colnames(RES)[i+7]) +
theme(legend.position = 'bottom') +
theme(text = element_text(size = 13, face = 'bold'), axis.title = element_text(size = 20, face = 'bold')) +
annotation_custom(grob1) +
annotation_custom(grob2) +
annotation_custom(grob3)
## COR ##
ggsave(paste('COR_DD_RD_', sprintf('%02d', i), '.jpg'), FIG, 'jpg', dpi = 600)
}